/**
 * A2 Module 6 (permission-requests) — module-local depth gate (Phase 3 Task 5).
 *
 * Four instructional lessons: pr1 permission-temoii intro (~てもいいです), pr2
 * prohibition-tewaikenai intro (~てはいけません), pr3 request-tekudasai intro
 * (~てください) + permission-temoii practice/recurrence, pr4 negative-request
 * intro (~ないでください) + prohibition-tewaikenai practice/recurrence.
 * Mirrors Module 1-5's rigor: assembles a real foundation catalog from the
 ...
 * >=5 unique visible targets (reuse <=2), and natural interaction/dialogue
 * content rather than an isolated grammar drill.
 */
...
import { module6Lessons, module6Recipe } from "./module06PermissionRequests";
...
describe("A2 Module 6 (permission-requests) — exactly 4 lessons, canonical order", () => {
  it("declares exactly permission-requests-1..4 in order 1..4", () => {
    expect(module6Lessons.map((b) => b.recipe.id)).toEqual([
      "permission-requests-1",
      "permission-requests-2",
      "permission-requests-3",
      "permission-requests-4",
    ]);
    expect(module6Lessons.map((b) => b.recipe.order)).toEqual([1, 2, 3, 4]);
    expect(module6Lessons.every((b) => b.recipe.moduleId === "permission-requests")).toBe(true);
  });

  it("declares the exact primary/support Can-do mapping for M6", () => {
    ...
    for (const built of module6Lessons) {
  ...
  it("declares module6Recipe with the exact 4 lesson ids", () => {
    expect(module6Recipe.lessonIds).toEqual([...module6Lessons.map((b) => b.recipe.id)]);
  });
});

describe("A2 Module 6 — per-lesson depth contract", () => {
  it.each(module6Lessons.map((b) => [b.recipe.id, b] as const))(
...
describe("A2 Module 6 — kanji exposure wiring", () => {
  it.each(module6Lessons.map((b) => [b.recipe.id, b] as const))(
  ...
  it("pr1 includes exactly the 4 new first-supported glyphs (入口出止)", () => {
    const pr1 = module6Lessons[0];
    const firstSupported = pr1.recipe.kanjiExposureIds.filter(
      (id) => A2_KANJI_EXPOSURES.find((e) => e.id === id)?.stage === "first-supported",
    );
    expect(firstSupported).toHaveLength(4);
  });

  it("pr2 includes exactly the 4 new first-supported glyphs (禁消座立)", () => {
    const pr2 = module6Lessons[1];
    const firstSupported = pr2.recipe.kanjiExposureIds.filter(
      (id) => A2_KANJI_EXPOSURES.find((e) => e.id === id)?.stage === "first-supported",
    );
    expect(firstSupported).toHaveLength(4);
  });
...
describe("A2 Module 6 — connected grammar content (no isolated drill)", () => {
  it("pr1's models are complete ~てもいい permission statements/questions", () => {
    const pr1 = module6Lessons[0];
    const models = pr1.variants.filter((v) => v.pedagogicalUse === "model");
    const sentences = models.map(realize);
    for (const s of sentences) {
      expect(s.canonicalJapanese).toContain("もいい");
    }
  });

  it("pr2's models are complete ~てはいけません prohibition statements", () => {
    const pr2 = module6Lessons[1];
    const models = pr2.variants.filter((v) => v.pedagogicalUse === "model");
    const sentences = models.map(realize);
    for (const s of sentences) {
      expect(s.canonicalJapanese).toContain("はいけません");
    }
  });

  it("pr3 mixes ~てください requests (intro) with ~てもいい permission recurrence", () => {
    const pr3 = module6Lessons[2];
    const models = pr3.variants.filter((v) => v.pedagogicalUse === "model");
    const sentences = models.map(realize);
  ...
  it("pr4 mixes ~ないでください negative requests (intro) with ~てはいけません prohibition recurrence", () => {
    const pr4 = module6Lessons[3];
    const models = pr4.variants.filter((v) => v.pedagogicalUse === "model");
    const sentences = models.map(realize);
    const negativeRequestCount = sentences.filter((s) => s.canonicalJapanese.includes("ないでください")).length;
    const prohibitionCount = sentences.filter((s) => s.canonicalJapanese.includes("はいけません")).length;
    expect(negativeRequestCount).toBeGreaterThanOrEqual(3);
    expect(prohibitionCount).toBeGreaterThanOrEqual(1);
  });
});

describe("A2 Module 6 — natural vocative use (never Xは for direct address)", () => {
  it("never marks a named individual referent as an explicit (は-marked) subject anywhere in M6 — direct address must be vocative or omitted", () => {
    const violations: string[] = [];
    for (const built of module6Lessons) {
      for (const variant of built.variants) {
        if (
          variant.discourse.subjectRealization === "explicit" &&
          (variant.discourse.subjectReferentId === "a2-referent-sora" ||
            variant.discourse.subjectReferentId === "a2-referent-emi")
        ) {
          violations.push(variant.id);
        }
      }
    }
    expect(violations, `explicit-subject named-individual violations: ${violations.join(", ")}`).toEqual([]);
  });
});

describe("A2 Module 6 — exact realized Japanese/rōmaji spot checks", () => {
  it("realizes every module-6 instructional variant through the shared formatter with no errors", () => {
    for (const built of module6Lessons) {
      for (const variant of built.variants) {
...
describe("A2 Module 6 — bilingual copy coverage", () => {
