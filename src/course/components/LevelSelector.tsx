 ...
 *
 * A visible `<p>` element renders `copy.selectorLabel` as the first child of
 * the `<nav>`. Its `id` is generated by `useId()` so that multiple instances
 * on the same page cannot collide. The landmark is named via
 * `aria-labelledby={labelId}` pointing at that element, so sighted and AT
 * users both see the same label text — no separate hidden aria-label is needed
 * (using both simultaneously would risk an ambiguous accessible name).
 *
