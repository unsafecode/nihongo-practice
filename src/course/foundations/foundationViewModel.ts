...
/**
 * Pure lesson view model for the compact sentence matrix, same-family guided
 * construction, and two-round practice UX (design spec §10.3, §11, §12;
 * Phase 1 Task 5). Given a fixture lesson id, a locale, and a deterministic
 * seed it re-derives the whole preview page — realizing every model and
 * practice candidate with the lesson's own available concepts, running the
 * production selector for both rounds, and generating every selected
 * exercise against one complete realized context — and returns a typed
 * success or a single typed error. It never returns a partial page.
 *
...
// ---------------------------------------------------------------------------
// Fixture-local instruction copy (harness-only; not production CourseCopy).
// Deliberately plain, encouraging wording — never certification/mastery.
// ---------------------------------------------------------------------------
...
/** The bilingual instruction map, exported so the harness can prove parity. */
export const foundationInstructionCopy = INSTRUCTION_COPY;

// ---------------------------------------------------------------------------
// Fixture-local semantic-axis labels (harness-only; not production CourseCopy).
// Plain descriptive wording so the guided board names each changing axis as
// text, never colour alone. Never certification/mastery language.
// ---------------------------------------------------------------------------
...
/** The bilingual axis-label map, exported so the harness can prove parity. */
...
function copyFor(locale: Locale, copyId: string): string {
  return foundationCopy[locale][copyId] ?? "";
}
...
export function buildFoundationLessonViewModel(
  ...
  const index = buildIndex(foundationCatalogs);
  const lesson = index.catalogs.lessons.find((l) => l.id === fixtureId);
  if (!lesson) {
    return { ok: false, error: { code: "unknown-lesson", detail: fixtureId } };
  }
  ...
  if (!modelSentences || !roundOneCandidates || !roundTwoCandidates) {
    return { ok: false, error: { code: "realization-failed", detail: fixtureId } };
  }
  ...
  const rowFor = (variantId: string): FoundationLocalizedRow | null => {
    ...
    return {
      ...
      translation: copyFor(locale, variantTranslationCopyId(variantId)),
      speaker: role ? copyFor(locale, role.labelCopyId) : "",
      context: context ? copyFor(locale, context.labelCopyId) : "",
    };
  };
  ...
  const toRoundTarget = (
    ...
    const intentText =
      target.exerciseKind === "constrained-construction" && sentence
        ? copyFor(locale, sentence.discourse.scenarioNoteCopyId)
        : null;
    return {
      ...
      translation: copyFor(locale, variantTranslationCopyId(target.variantId)),
  ...
  return {
    ...
    model: {
      ...
      canDoDescriptor: primaryCanDo
        ? copyFor(locale, primaryCanDo.descriptorCopyId)
        : "",
      matrix: {
        rows: matrixRows,
