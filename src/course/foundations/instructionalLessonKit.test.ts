...
import { validateFoundations, type ValidationError } from "./validateFoundations";
import {
  A1_INSTRUCTIONAL_KIT_CONFIG,
} from "../a1/catalog/shared";
import {
  a1SemanticFoundationCatalogs,
  a1FoundationCopy,
} from "../a1/catalog/catalog";
import { A1_RELEASE_CATALOG_VERSION, A1_RELEASE_SEED } from "../a1/releaseIdentity";
...
describe("declared model-family floor (Phase 4 Task 4)", () => {
  ...
  // -------------------------------------------------------------------------
  // Production-path rejection proof (D8): kit config → buildInstructionalLesson
  // → diversityConstraints → validateFoundations → insufficient-family-diversity.
  //
  // 19c5988 proved *propagation* (the config value reaches diversityConstraints).
  // This test proves *rejection*: the gate fires through the real production
  // path, using the real A1 release catalogs — not a fake fixture, and not a
  // direct patch on an already-built lesson.
  // -------------------------------------------------------------------------

  it("production path: validateFoundations rejects when kit config declares minFamilies: 2 on single-family lessons", () => {
    ...
    const built = buildInstructionalLesson(raisedConfig, {
      id: "actions-1",
      moduleId: "actions",
      order: 1 as const,
    ...
    // Convert the kit-built lesson into a FoundationLessonDefinition (the
    // shape validateFoundations consumes), then swap it into the real A1
    // release catalogs — replacing only the "actions-1" entry so every
    // earlier validation stage passes on the unmodified 43 siblings.
    const variantById = new Map(
      a1SemanticFoundationCatalogs.sentenceVariants.map((v) => [v.id, v]),
    );
    const raisedLessonDef = toFoundationLessonDefinition(built.recipe, "a1", variantById);
    ...
    const raisedResult = validateFoundations({
    ...
    // The gate must fire: actions-1 has 1 family but the kit declared minFamilies: 2.
    expect(raisedResult.valid).toBe(false);
    const familyErrors = raisedResult.errors.filter(
    ...
    const actions1Error = familyErrors.find(
      (e: ValidationError) => "lessonId" in e && (e as unknown as Record<string, unknown>).lessonId === "actions-1",
    );
    expect(actions1Error, "actions-1 must be rejected for insufficient family diversity").toBeDefined();

    // Companion: the REAL shipped catalogs (minFamilies: 1) produce zero
    // insufficient-family-diversity errors — so the test discriminates.
    ...
    const shippedFamilyErrors = shippedResult.errors.filter(
      (e: ValidationError) => e.code === "insufficient-family-diversity",
    );
    expect(shippedFamilyErrors, "shipped floor of 1 must produce zero family-diversity errors").toHaveLength(0);
  });
});
